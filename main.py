...
video_details_path = save_video_details(video_id, os.path.join(dist, "details.json"))
...
explorer = HotExplorer("jZ_tUpwPyBA/chats.json")
...
with ProcessPoolExecutor(max_workers=6) as excecutor:
    excecutor.map(
        ...
        [(clip, video_id, os.path.join(dist, f"{clip.start}-{clip.end}.mp4")) for clip in clips]
    )
...
hotclips_path = concat_videos(videos_text_path, os.path.join(dist, "hotclips.mp4"))
